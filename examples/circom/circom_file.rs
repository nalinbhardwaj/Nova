...
pub(crate) fn read_field<R: Read, Fr: PrimeField>(mut reader: R) -> Result<Fr> {
    ...
    for digit in repr.as_mut().iter_mut() { // TODO: may need to reverse order?
...
fn read_constraint_vec<R: Read, Fr: PrimeField>(mut reader: R, header: &Header) -> Result<Vec<(usize, Fr)>> {
...
fn read_constraints<R: Read, Fr: PrimeField>(mut reader: R, size: u64, header: &Header) -> Result<Vec<Constraint<Fr>>> {
...
pub fn from_reader<R: Read + Seek>(mut reader: R) -> Result<R1CSFile<<Bls12 as Engine>::Fr>> {
    let mut magic = [0u8; 4];
    ...
    let constraints = read_constraints::<&mut R, <Bls12 as Engine>::Fr>(&mut reader, *section_sizes.get(&constraint_type).unwrap(), &header)?;
