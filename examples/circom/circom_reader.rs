...
use crate::circom_file::read_field;
use ff::PrimeField;
use blstrs::Bls12;
use pairing::Engine;
use hex_literal::hex;
...
fn load_witness_from_json<Fr: PrimeField, R: Read>(reader: R) -> Vec<Fr> {
    ...
    witness.into_iter().map(|x| Fr::from_str_vartime(&x).unwrap()).collect::<Vec<Fr>>()
}

/// load witness from bin file by filename
pub fn load_witness_from_bin_file<Fr: PrimeField>(filename: &str) -> Vec<Fr> {
    let reader = OpenOptions::new().read(true).open(filename).expect("unable to open.");
    load_witness_from_bin_reader::<Fr, BufReader<File>>(BufReader::new(reader)).expect("read witness failed")
}
...
fn load_witness_from_bin_reader<Fr: PrimeField, R: Read>(mut reader: R) -> Result<Vec<Fr>, anyhow::Error> {
...
pub fn load_r1cs(filename: &str) -> R1CS<<Bls12 as Engine>::Fr> {
    if filename.ends_with("json") {
...
fn load_r1cs_from_bin_file(filename: &str) -> (R1CS<<Bls12 as Engine>::Fr>, Vec<usize>) {
    let reader = OpenOptions::new().read(true).open(filename).expect("unable to open.");
    load_r1cs_from_bin(BufReader::new(reader))
}

/// load r1cs from bin by a reader
fn load_r1cs_from_bin<R: Read + Seek>(reader: R) -> (R1CS<<Bls12 as Engine>::Fr>, Vec<usize>) {
    let file = crate::circom_file::from_reader(reader).expect("unable to read.");
...
mod tests {
    use crate::circom_reader::{load_r1cs, load_witness_from_file};
